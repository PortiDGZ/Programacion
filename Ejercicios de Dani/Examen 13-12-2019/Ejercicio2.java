...
public class Ejercicio2 {
	...
	public static long cambiarDigito(long n, long pos) {
		// Cambiamos por 0 la posición deseada y todas las de la izquierda
		long resultado = voltearNum(n);
		resultado = (long) (resultado / Math.pow(10, pos)) * (long) Math.pow(10, pos);

		// Generamos la cifra
		long cifra = (long) (Math.random() * 10);

		// Colocamos la cifra
		resultado += cifra * (long) Math.pow(10, pos - 1);

		// Obtenemos los números a la izquierda de la cifra
		resultado = voltearNum(resultado);
		long l = longitud(resultado);
		long izquierda = (long) (n / Math.pow(10, l)) * (long) Math.pow(10, l);

		// Colocamos los números a la izquierda de la cifra
		resultado += izquierda;
	...
	public static void main(String[] args) {
		...
		System.out.println(cambiarDigito(14789, 4));
